...
cd Underwater-Navigation-System
python3 -m venv .venv
...
A real-time Python simulation of the navigation subsystem (with sensing and communication elements) designed for a deep-sea research submarine operating in the Clarion Clipperton Zone (CCZ). It visualises how a stacked navigation system involving an Inertial Navigation System (INS), Doppler Velocity Log (DVL), and Acoustic Doppler Velocity Profiler (ADCP), accumulates position error over a multi-day survey mission, and how a GPS buoy corrects that drift.
...
* Layered navigation error model (INS baseline, DVL water-track aiding, ADCP current correction) based on CEP drift-rate parameters derived from the underlying MATLAB analysis
...
|snc_main.py|Wires the modules together and drives the live animation|
...
The simulator runs entirely on matplotlib and numpy 

## Deep Sea Survey - Interactive Game 
...
### Features
* Gamepad Support: PS4 controller input via pygame, blended with keyboard controls (if controller unavailable)
...
```
For grower game version: https://shreelakshminarasimhan.github.io/Underwater-Navigation-System/
